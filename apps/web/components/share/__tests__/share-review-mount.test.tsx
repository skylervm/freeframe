...
describe('folder share review on phones', () => {
  ...
  it('uses the review page layout hooks instead of a scrolling page with a fixed video box', async () => {
    ...
    // Comments take the remaining height and scroll inside themselves.
    expect(comments).toHaveClass('flex-1', 'min-h-0', 'overflow-hidden', 'md:w-[360px]', 'md:flex-none')
    ...
    // iOS Safari's 100vh hides the composer under its toolbar.
    expect(workspace.parentElement).toHaveClass('h-[100svh]', 'md:h-screen')
  })
...
describe('folder share review — a ready video on phones', () => {
  ...
  it('sizes the video to its own aspect ratio while comments are open, and fills the column when they close', async () => {
    ...
    // Closing comments: the header toggle is the button just before ⋯.
    const options = screen.getByRole('button', { name: 'Comment options' })
    fireEvent.click(options.previousElementSibling as HTMLElement)
