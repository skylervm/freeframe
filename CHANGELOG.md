...
### Fixed
- **The review workspace adapts to phone orientation** — in landscape below 768px the review surface is now a real two-column workspace with the media on the left and Comments/Fields on the right, instead of a stacked mobile layout that squeezed the video to almost nothing. In portrait the video sits in a box matching its own aspect ratio rather than a tall viewport-height container that added black letterbox bars above and below it, so the comments pane and composer get the remaining space. A vertical or square clip gets its own shape instead of being pillarboxed into 16:9; 16:9 is used only as the fallback until the source's dimensions are known. Desktop (`md+`) review is unchanged. (#18)
- **The video timeline is easier to scrub on a phone** — the progress track is inset from the physical screen edges and sits inside a much taller pointer target, and seek time is mapped from the visual track and clamped, so a press in either inset gutter reliably reaches the start or the end of the clip. The desktop timeline keeps its current geometry. (#18)
