...
### Fixed
- **The review workspace adapts to phone orientation** — in landscape below 768px the review surface is now a real two-column workspace with the media on the left and Comments/Fields on the right, instead of a stacked mobile layout that squeezed the video to almost nothing. In portrait the video sits in its natural 16:9 frame rather than a tall viewport-height container that added black letterbox bars above and below it, so the comments pane and composer get the remaining space. Desktop (`md+`) review is unchanged. (#18)
